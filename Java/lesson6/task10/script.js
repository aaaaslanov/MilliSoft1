...
let people = ['Michael', 'Kate', 'ALex', 'John', 'Lipa'];
let table_head = `<tr>`;
for(let i = 0; i < 3; i++) {table_head += `<th class="${i}"><button class="blue ieqb" onclick="allIe(${i})">i/e</button></th>`}
table_head += `</tr><tr>`;
for(let i = 0; i < 3; i++) {
table_head += `<th class="${i}"><button class="red ieqb" onclick="allQb(${i})">q/b</button></th>`;}
table_head += `</tr>`;thead.innerHTML += table_head;
people.forEach((person, index) => {
    let table_row = `<tr>
        <td>${index + 1}</td><td>${person}</td>`;
    for(let i = 0; i < 3; i++) {
        table_row += `<td class="${i}">
            <button class="td-button de" onClick="openModal(${index}, ${i})">d/e</button>
        </td>`;
    }
    table_row += `</tr>`; t_body.innerHTML += table_row;
});
function openModal(index, cell) {modal.className = index; modal.setAttribute('cell', cell); modal.style.display = 'flex';}
function changeGrade(i) {
    ...
    grade_btn.textContent = i; grade_btn.disabled = true;
    grade_btn.removeAttribute('onClick'); modal.style.display = 'none';
}
    ...
    td_btn.forEach((tdButton, i) => {
        if(tdButton.parentElement.className == index) {
            if(tdButton.className == 'td-button de') {
                tdButton.className = 'td-button blue'; tdButton.textContent = 'i/e';
                tdButton.disabled = true; tdButton.removeAttribute('onClick');
            }
        }
    })
}
function allQb(index) {
    ...
    td_btn.forEach((tdButton, i) => {
        if(tdButton.parentElement.className == index) {
            if(tdButton.className == 'td-button de') {
                tdButton.className = 'td-button red'; tdButton.textContent = 'q/b';
                tdButton.disabled = true; tdButton.removeAttribute('onClick');
            }
        }
    })
